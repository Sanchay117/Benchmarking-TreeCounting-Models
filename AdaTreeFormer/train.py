...
parser.add_argument('--data-dir', default='/home/ashank/TreeCounting_Benchmark/TreeFormer/datasets', help='London dataset root path')
parser.add_argument('--data-dir-shot', default='/home/ashank/TreeCounting_Benchmark/TreeFormer/datasets', help='London dataset root path for shot branch')
parser.add_argument('--save-root', default='/home/ashank/TreeCounting_Benchmark/AdaTreeFormer/ckpts', help='root directory for checkpoints and logs')
parser.add_argument('--pretrained-path', default='', help='path to SwinV2 pretrained checkpoint (.pth); leave empty to use random backbone init')
parser.add_argument('--train-split', default='train_data', help='source training split folder name')
parser.add_argument('--val-split', default='valid_data', help='source validation split folder name (not test split)')
parser.add_argument('--shot-train-split', default='train_data', help='shot training split folder name')
parser.add_argument('--shot-val-split', default='valid_data', help='shot validation split folder name (not test split)')
...
parser.add_argument('--batch-size-shot', type=int, default=5, help='train batch size')
...
parser.add_argument('--run-name', default='New', help='run name for wandb interface/logging')
...
class Trainer(object):
    ...
    def setup(self):
        ...
        self.save_dir = os.path.join(args.save_root, sub_dir)
        ...
        def resolve_train_split(root_dir, split_name, branch_name):
            split_path = os.path.join(root_dir, split_name)
            fallback_split = "train_data"
            fallback_path = os.path.join(root_dir, fallback_split)
            split_ok = os.path.isdir(split_path) and len(os.listdir(split_path)) > 0
            if split_ok:
                return split_name

            fallback_ok = os.path.isdir(fallback_path) and len(os.listdir(fallback_path)) > 0
            if fallback_ok:
                self.logger.warning(
                    "%s split '%s' is missing/empty at %s; falling back to '%s'.",
                    branch_name,
                    split_name,
                    split_path,
                    fallback_split,
            ...
            raise ValueError(
                "No usable training split for {}. Checked '{}' ({}) and '{}' ({}).".format(
                    branch_name,
                    split_name,
                    split_path,
                    fallback_split,
        ...
        train_split = resolve_train_split(args.data_dir, args.train_split, "source")
        shot_train_split = resolve_train_split(args.data_dir_shot, args.shot_train_split, "shot")
        
        
        downsample_ratio = args.scale_factor
        self.datasets = {"train": Crowd_TC(os.path.join(args.data_dir, train_split), args.crop_size,
            downsample_ratio, "train"), "val": Crowd_TC(os.path.join(args.data_dir, args.val_split),
                args.crop_size, downsample_ratio, "val")}
        # 
        self.datasets_shot = { "train": Crowd_TC(os.path.join(args.data_dir_shot, shot_train_split), 
            args.crop_size, downsample_ratio, "train"),"val": Crowd_TC(os.path.join(args.data_dir_shot, args.shot_val_split),
                args.crop_size, downsample_ratio, "val")}
        ...
        self.dataloaders_shot = {
            x: DataLoader(self.datasets_shot[x],
                ...
                batch_size=(args.batch_size_shot if x == "train" else 1),
                shuffle=(True if x == "train" else False),
                num_workers=args.num_workers * self.device_count,
                pin_memory=(True if x == "train" else False))
            for x in ["train", "val"]}

        pretrained_path = args.pretrained_path.strip()
        if not pretrained_path:
            default_pretrained = os.path.join(os.path.dirname(__file__), 'swinv2_base_patch4_window12to16_192to256_22kto1k_ft.pth')
            pretrained_path = default_pretrained if os.path.isfile(default_pretrained) else ''

        self.model = QKV('swin', pretrained_path, False)
    ...
    def train_epoch(self):
        ...
        for step, (inputs, gausss, points, gt_discrete) in enumerate(self.dataloaders["train"]):
            ...
            with torch.set_grad_enabled(True): 
                ...
                DAN_target = torch.tensor([0] * (args.batch_size + args.batch_size_shot)).cuda()
